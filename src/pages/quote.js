...
export function quote() {
  ...
  return `<section class="quote-page"><div class="quote-heading"><button class="back" data-page="home">← Accueil</button>${eyebrow('DÉFINIR VOTRE BESOIN')}<h1>Une solution juste.<br><em>Commençons par vous.</em></h1><p>Votre application est le point de départ.<br>Préparons un échange avec Maturat Mesure.</p></div><div class="quote-layout"><aside class="quote-aside"><ol class="form-progress">${progressItems}</ol><div class="side-summary"><span class="mono">VOTRE APPLICATION</span><div id="live-summary">${summary(true)}</div></div><p class="privacy-note">Mode démonstration. Vos saisies restent dans cette page jusqu'à son rechargement. Aucun envoi réel.</p><p class="quote-aside-help"><a href="#/faq">Questions fréquentes <span aria-hidden="true">↗</span></a></p></aside><form class="form-card">${fields()}</form></div></section>`;
}
